...
if exist "%~dp0ClaudeRestart.exe" (
    "%~dp0ClaudeRestart.exe" --remove-automation --pause
    exit /b
)
...
if not errorlevel 1 (
    py.exe -3 "%~dp0claude_restart.py" --remove-automation --pause
    exit /b
)

where python.exe >nul 2>&1
if not errorlevel 1 (
    python.exe "%~dp0claude_restart.py" --remove-automation --pause
    exit /b
)

...
| A rollback whose task delete fails says so instead of reporting the task removed | tests/test_install_transaction.py::test_a_rollback_delete_that_fails_is_reported_not_claimed |
...
| An event log that cannot be read is an error, never no events, so a launch reports VISIBLE rather than GREEN, a trace cannot report CLEAR, and an event-triggered run does not report NO ACTION | tests/test_events.py::test_a_denied_log_is_an_error_not_an_empty_answer, tests/test_events.py::test_a_missing_log_is_an_error_even_though_it_is_also_not_found, tests/test_events.py::test_a_query_that_matched_nothing_is_no_events, tests/test_events.py::test_the_real_log_is_read_without_error, tests/test_cli.py::test_an_event_triggered_run_reports_the_error_instead_of_no_action, tests/test_cli.py::test_a_trace_reports_the_error_instead_of_clear, tests/test_dialogs.py::test_visible_without_an_event_check_leaves_them_open |
